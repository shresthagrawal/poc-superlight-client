...
export class MerkleTree {
  ...
  init(leaves: Uint8Array[]) {
    ...
    if(!l)
      throw new Error(`there should be atleast one leaf`);

    if(l !== this.n ** logFloor(l, this.n))
      throw new Error(`leaves should be exact power of n(${this.n})`);
    
    let lastLayer: Node[] = leaves.map(l => ({hash: l, isLeaf: true, isRoot: false }));
  ...
  getRoot(dept: number = 0) {
    ...
    for(let i = 0; i < dept; i++) {
      if(!root.children)
        throw new Error('dept too big for the tree');
