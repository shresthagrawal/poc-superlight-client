...
export class SuperlightSync<T> {
  ...
  getLeafWithProof(period: number | "latest"): {syncCommittee: string[], root: string, proof: string[][]} {
    if (period === "latest")
  ...
  getMMRInfo(): {root: string, trees: {root: string, size: number}[]} {
    ...
    return {
      root: toHexString(rootHash),
      trees: treeInfo.map(i => ({root: toHexString(i.rootHash), size: i.size}))
    }
  }

  getNode(treeRootHex: string, nodeHashHex: string): {isLeaf: boolean, children?: string[]} {
